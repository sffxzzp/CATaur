...
export class ApplicationsService {
  ...
  async findAll(
    ...
    if (scope.role === Role.CLIENT) {
      if (scope.companyIds?.length) {
        qb.andWhere('jobOrder.companyId IN (:...cids)', {
          cids: scope.companyIds,
        });
      } else {
        qb.andWhere('1=0');
      }
    ...
    if (searchTerm) {
      qb.andWhere(
        '(candidate.nickname LIKE :s OR candidate.email LIKE :s OR jobOrder.title LIKE :s)',
        {
          s: `%${searchTerm}%`,
        },
      );
  ...
  async findDecisions(
    ...
    const pendingCount = await qb
      .clone()
      .andWhere('app.status = :status', { status: 'interview' })
      .getCount();
    const requestOfferCount = await qb
      .clone()
      .andWhere('app.clientDecisionType = :dt', { dt: 'request-offer' })
      .getCount();
    const passCount = await qb
      .clone()
      .andWhere('app.clientDecisionType = :dt', { dt: 'pass' })
      .getCount();
    const holdCount = await qb
      .clone()
      .andWhere('app.clientDecisionType = :dt', { dt: 'hold' })
      .getCount();
  ...
  async create(
    ...
    this.logger.log(
      `Application created: ${app.id} (${source}) candidate=${dto.candidateId} job=${dto.jobOrderId}`,
    );
  ...
  async bulkImport(dto: BulkImportDto): Promise<Application[]> {
    ...
    for (const c of dto.candidates) {
      ...
      let candidate = await this.userRepo.findOne({
        where: { email: c.email },
      });
      if (!candidate) {
    ...
    this.logger.log(
      `Bulk import: ${results.length} applications created for job ${dto.jobOrderId}`,
    );
    return results;
  }
  ...
  async updateStatus(
    ...
    if (dto.status === 'interview') {
      if (!dto.interviewDate || !dto.interviewContent) {
        throw new BadRequestException(
          'Interview date and content are required',
        );
      }
      app.interviewSubject = dto.interviewSubject
        ? (this.encryptionService.encryptText(
      ...
      app.interviewContent = this.encryptionService.encryptText(
        dto.interviewContent,
      ) as unknown as string;
      ...
      await this.emailService
        ...
        .catch((err) => {
          this.logger.error(
            `Failed to send interview email for application ${id}: ${err?.message}`,
          );
        });

      // Create in-app notification for candidate
      const when =
        `${dto.interviewDate ?? ''}${dto.interviewTime ? ` ${dto.interviewTime}` : ''}`.trim();
      ...
      await this.notificationsService
        .create(app.candidateId, 'interview_scheduled', title, body, app.id)
        .catch((err) => {
          this.logger.error(
            `Failed to create interview notification for candidate (app ${id}): ${err?.message}`,
          );
        });
    }
    ...
    if (dto.status === 'offer' && prevStatus !== 'offer') {
      ...
      await this.notificationsService
        ...
        .catch((err) => {
          this.logger.error(
            `Failed to create offer notification for candidate (app ${id}): ${err?.message}`,
          );
        });
  ...
  async submitDecision(
    ...
    this.logger.log(
      `Client decision "${dto.type}" received for application ${id}`,
    );
    return this.decryptApplication(app);
  }
  ...
  async updateApplicationCandidate(
    ...
    if (dto.locationCountry !== undefined)
      app.locationCountry = dto.locationCountry ?? null;
    if (dto.locationState !== undefined)
      app.locationState = dto.locationState ?? null;
    if (dto.locationCity !== undefined)
      app.locationCity = dto.locationCity ?? null;
  ...
  private async getApplication(
    ...
    if (
      ...
      !scope.companyIds.includes(app.jobOrder?.companyId ?? '')
    ) {
  ...
  private decryptApplication(application: Application): Application {
    ...
    application.interviewSubject = Buffer.isBuffer(application.interviewSubject)
      ? (this.encryptionService.decryptText(
    ...
    application.interviewContent = Buffer.isBuffer(application.interviewContent)
      ...
      : application.interviewContent;
    application.clientDecisionNote = Buffer.isBuffer(
    ...
    if (
      ...
      Buffer.isBuffer(application.candidate.phone)
    ) {
    ...
    if (
      ...
      Buffer.isBuffer(application.jobOrder.salary)
    ) {
    ...
    if (
      application.jobOrder?.location &&
      Buffer.isBuffer(application.jobOrder.location)
    ) {
    ...
    if (
      application.jobOrder?.company?.email &&
      Buffer.isBuffer(application.jobOrder.company.email)
    ) {
    ...
    if (
      application.jobOrder?.company?.phone &&
      Buffer.isBuffer(application.jobOrder.company.phone)
    ) {
